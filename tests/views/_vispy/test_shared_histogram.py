...
@fixture
def hist() -> VispySharedHistogramCanvas:
    canvas = VispySharedHistogramCanvas()
    canvas.set_range(x=(0, 100), y=(0, 1))
    return canvas


def _world_to_canvas(
    hist: VispySharedHistogramCanvas, x: float, y: float
) -> tuple[float, float]:
...
def test_channel_creation(hist: VispySharedHistogramCanvas) -> None:
...
@pytest.mark.usefixtures("any_app")
def test_channel_removal(hist: VispySharedHistogramCanvas) -> None:
...
@pytest.mark.usefixtures("any_app")
def test_channel_visibility(hist: VispySharedHistogramCanvas) -> None:
    ...
    assert ch.lut_line.visible is False
