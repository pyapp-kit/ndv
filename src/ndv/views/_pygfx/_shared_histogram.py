...
class PyGFXSharedHistogramCanvas(SharedHistogramCanvas):
    ...
    def __init__(self) -> None:
        ...
        # Legend texts are added to a separate overlay scene
        self._legend_scene = pygfx.Scene()
        self._legend_cam = pygfx.OrthographicCamera(
            maintain_aspect=False, width=1, height=1
        )
        self._legend_cam.local.position = [0.5, 0.5, 0]
    ...
    def on_mouse_move(self, event: MouseMoveEvent) -> bool:
        ...
        if self._grabbed is Grabbable.GAMMA:
            y_max = self._channel_y_max(ch)
            y = self.canvas_to_world(pos)[1]
            if y <= 0 or y_max <= 0:
                return False
            gamma = max(MIN_GAMMA, -np.log2(y / y_max))
    ...
    def _ensure_channel(self, key: object) -> _ChannelVisuals:
        ...
        area_mesh = pygfx.Mesh(
            ...
            material=pygfx.MeshBasicMaterial(color=(0.5, 0.5, 0.5, FILL_ALPHA)),
        )
        ...
        outline = pygfx.Line(
            ...
            material=pygfx.LineMaterial(color="white", thickness=1.5),
        )
    ...
    def _update_channel_area(self, key: object) -> None:
        ...
        if outline_pos.shape == ch.outline.geometry.positions.data.shape:
            ch.outline.geometry.positions.data[:] = outline_pos
            ch.outline.geometry.positions.update_range()
        else:
            ch.outline.geometry = pygfx.Geometry(positions=outline_pos)
    ...
    def _update_lut_visuals(self, key: object, npoints: int = 64) -> None:
        ...
        # Left clim line
        left_pos = np.array([[clims[0], 0, 0], [clims[0], y_max, 0]], dtype=np.float32)
        self._update_line_positions(ch.left_clim, left_pos)
        ch.left_clim.visible = ch.visible

        # Right clim line
        right_pos = np.array([[clims[1], 0, 0], [clims[1], y_max, 0]], dtype=np.float32)
    ...
    def _resize(self, x: tuple[float, float], y: tuple[float, float]) -> None:
        ...
        self._x_cam.width = self._camera.width
    ...
    def _update_legend(self) -> None:
        """Update legend texts in the legend scene."""
        # For now, legends are not yet rendered via pygfx text
        # (would need MultiText per channel). This can be added later.
        pass
